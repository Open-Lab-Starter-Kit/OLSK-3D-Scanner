...
export default function Model({ modelIn, modelOut, modelInCopy, modelInCopy2, modelOutCopy }) {
    ...
    const preparingStepArray = [ //shown grouped
        [
            "021_Prepare_the_camera_head",
            "022_Fix_camera_light",
            "023_Prepare_head_side_plate",
            "024_Fix_the_camera"
        ],
        [
            "041_Prepare_the_frame_base_1",
            "042_Prepare_the_frame_base_2",
            "043_Prepare_the_frame_base_3",
            "044_Prepare_the_frame_base_4"
        ],
        [
            "061_Prepare_the_frame_top_1",
            "062_Prepare_the_frame_top_2"
        ],
        [
            "091_Prepare_electronic_panel_1",
            "092_Prepare_electronic_panel_2"
        ],
        [
            "131_Prepare_background_panels_1"
        ],
        [
            "132_Prepare_background_panels_2"
        ]
    ]
    ...
    const mainMachineBuildStepChange = useCallback(() => {
        ...
        mainMachineBuildArray.filter(obj => !exceptionArray.some(arr => arr.includes(obj.name)))
            .forEach(obj => {
                obj.visible = true;
                let clonedObj = obj.clone();
                        ...
                        if (mesh.isGroup) {
                            for (let i = 0; i < mesh.children.length; i++) {
                                if (mesh.children[i].isMesh) {
                                    mesh.children[i].frustumCulled = false //fixes disappearing faces
